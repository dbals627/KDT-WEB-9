...
app.post('/hash', (req, res) => {
    const { pw } = req.body;
    //const hash = createHashedPassword(pw)  //createHashedPassword는 리턴함수이기 때문에 변수값에 들어갈 수 있음
    //hash = createPbkdf(pw);
...
const decipher = (encryptedData) => {
    const decipher = crypto.createDecipheriv(algorithm, key, iv);  //복호화 객체를 생성 
    let decrypedData = decipher.update(encryptedData, 'base64', 'utf-8');  //base64;입력인코딩 , utf-8;출력인코딩
