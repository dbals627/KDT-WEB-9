...
export const post_signup = async (data) => {
    try {
        const query = 'INSERT INTO prac (userid, pw, name) VALUES (?, ?, ?)';
...
export const post_signin = async (data) => {
    try {
        const query = 'SELECT * FROM prac WHERE userid = ? AND pw = ?';
...
export const post_profile = async (data) => {
    try {
        const query = 'SELECT * FROM prac WHERE userid = ?';
...
export const edit_profile = async (data) => {
    try {
        const query = 'UPDATE prac SET userid=?, pw=?, name=? WHERE id= ?';
...
export const delete_profile = async (id) => {
    try {
        const query = 'DELETE FROM prac WHERE id = ?';
